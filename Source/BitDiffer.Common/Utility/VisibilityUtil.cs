...
namespace BitDiffer.Common.Utility
{
    public static class VisibilityUtil
    {
        ...
        public static ChangeType GetVisibilityChange(Visibility from, Visibility to, bool suppressBreakingChanges, bool isVirtual = false)
        {
            if (from == to)
            {
            ...
            {
                return ChangeType.VisibilityChangedBreaking; // any visibility change is breaking for the virtual member, as it can be overriden.
            }


            if ((!suppressBreakingChanges) && (from == Visibility.Public || (from == Visibility.Protected && to != Visibility.Public))) // increasing visibility is not a breaking change for non-virtual members
            {
