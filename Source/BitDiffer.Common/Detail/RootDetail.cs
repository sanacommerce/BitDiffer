...
namespace BitDiffer.Common.Model
{
    ...
    public class RootDetail : AlignableBase, ICanCompare, IComparable<RootDetail>, IComparable
    {
        ...
        protected ChangeType PerformCompareInternal(ICanCompare from, bool suppressBreakingChanges)
        {
            ...
            {
                if (this.Parent is InterfaceDetail || (this.Parent is ClassDetail && this.GetTextDeclaration().Contains("abstract ")))
                    _changeThisInstance = ChangeType.AddedBreaking;
                else
        ...
        protected virtual void SerializeWriteContent(XmlWriter writer)
        {
            ...
            if (previous != null)
            {
                ...
                if (this.Change != ChangeType.Added && this.Change != ChangeType.AddedBreaking)
                {
        ...
        protected virtual string SerializeGetWhatChangedName()
        {
            ...
            if ((this.Change & ChangeType.Added) != 0 || (this.Change & ChangeType.AddedBreaking) != 0)
            {
                sb.Append("Added, ");
            }
        ...
        {
            if (((changeType & ChangeType.DeclarationChangedNonBreaking) != 0) || ((changeType & ChangeType.DeclarationChangedBreaking) != 0) || (changeType == ChangeType.Added || changeType == ChangeType.AddedBreaking) || (changeType == ChangeType.ValueChangedBreaking) || (changeType == ChangeType.ValueChangedNonBreaking))
            {
                return ToString();
            }
